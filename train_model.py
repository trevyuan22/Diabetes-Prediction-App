...
for feat in zero_features:
    ...
    df[feat] = df[feat].fillna(median_val)   # direct assignment, no chained warning
...
# ----------------------------
# 5. SMOTE + Hyperparameter Tuning (single-threaded for Windows stability)
# ----------------------------
...
try:
    ...
    print(f"\n⚠️ SHAP plotting failed: {e}")
    # Create placeholder images
    fig, ax = plt.subplots()
    ax.text(0.5, 0.5, "SHAP unavailable\n(but model works fine!)", ha='center', va='center')
    ax.axis('off')
    fig.savefig('shap_summary.png', dpi=100, bbox_inches='tight')
    fig.savefig('shap_bar.png', dpi=100, bbox_inches='tight')
    plt.close(fig)
